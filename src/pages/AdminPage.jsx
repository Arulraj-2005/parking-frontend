...
function StatCard({ label, value, note }) {
  return (
    <div className="rounded-[26px] border border-[#e7ddd3] bg-white p-5 shadow-sm">
...
function CompletedRow({ session }) {
  return (
    <tr className="border-b border-[#efe5db]">
      ...
      <td className="px-4 py-3 text-sm text-stone-500">{session.duration_hours}h</td>
      <td className="px-4 py-3 text-sm text-stone-500">{fmtDateTime(session.entry_time)}</td>
      <td className="px-4 py-3 text-sm font-semibold text-[#2f6f57]">${session.total_amount?.toFixed(2) || '0.00'}</td>
    </tr>
  );
}
...
function UserRow({ rowUser, currentUserId, onRoleChange, onDelete }) {
  return (
    <tr className="border-b border-[#efe5db]">
      <td className="px-4 py-3 text-sm font-semibold text-stone-800">{rowUser.full_name}</td>
      <td className="px-4 py-3 text-sm text-stone-500">@{rowUser.username}</td>
      <td className="px-4 py-3 text-sm text-stone-500">{rowUser.email}</td>
      <td className="px-4 py-3 text-sm text-stone-500">{rowUser.phone || '—'}</td>
      <td className="px-4 py-3 text-sm capitalize text-stone-600">{rowUser.role}</td>
        ...
        {rowUser.id !== currentUserId ? (
          <div className="flex items-center gap-2">
            <select
              ...
              className="rounded-xl border border-[#ddcfc2] bg-white px-3 py-2 text-xs text-stone-700 outline-none"
            >
              <option value="customer">Customer</option>
              <option value="staff">Staff</option>
              <option value="admin">Admin</option>
            </select>
            <button onClick={() => onDelete(rowUser.id)} className="rounded-xl bg-[#f7e8eb] px-3 py-2 text-xs font-semibold text-[#8b3b4b]">
              Remove
            </button>
          </div>
        ) : (
          <span className="text-xs text-stone-400">Current user</span>
        )}
      </td>
    </tr>
  );
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f7f2ec]">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm uppercase tracking-[0.22em] text-stone-500">Loading admin panel</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#f6f1ea] text-stone-900">
      ...
      <header className="sticky top-0 z-40 border-b border-[#e6d9cc] bg-[#fbf8f3]/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div>
            <p className="text-[11px] uppercase tracking-[0.24em] text-stone-500">Administration</p>
            <h1 className="text-2xl font-semibold">Smart Parking Control</h1>
          </div>
          <div className="flex items-center gap-3">
            {criticalCount > 0 && <span className="rounded-full bg-[#f6d4db] px-3 py-1.5 text-xs font-semibold text-[#8b3b4b]">{criticalCount} critical</span>}
            {warningCount > criticalCount && <span className="rounded-full bg-[#f7e8c8] px-3 py-1.5 text-xs font-semibold text-[#9a6a28]">{warningCount} expiring</span>}
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-medium transition ${
                activeTab === tab.id ? 'bg-[#7c4f64] text-white' : 'bg-white text-stone-600 border border-[#e6d9cc]'
              }`}
            >
              {tab.label}
            </button>
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        {activeTab === 'dashboard' && stats && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.05fr_0.95fr]">
              <div className="rounded-[28px] border border-[#e7ddd3] bg-white p-6 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-semibold">Live occupancy</h2>
                  <span className="text-sm text-stone-500">{stats.occupancyRate}% used</span>
                </div>
                <div className="h-4 rounded-full bg-[#efe7de]">
                  <div className="h-4 rounded-full bg-[#7c4f64]" style={{ width: `${stats.occupancyRate}%` }} />
                </div>
                ...
                <div className="mt-4 space-y-4">
                  {zones.map((zone) => (
                    <div key={zone.id}>
                      ...
                      <div className="h-2.5 rounded-full bg-[#efe7de]">
                        <div className="h-2.5 rounded-full bg-[#465055]" style={{ width: `${(zone.occupied_spots / zone.total_spots) * 100}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="overflow-hidden rounded-[28px] border border-[#e7ddd3] bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-[#efe5db] px-6 py-4">
                ...
                <span className="text-sm text-stone-500">{activeSessions.length} active</span>
              </div>
        ...
        {activeTab === 'parking' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[11px] uppercase tracking-[0.22em] text-stone-500">Parking map</p>
                <h2 className="text-2xl font-semibold">Zone-wise slot layout</h2>
              </div>
              <button onClick={fetchData} className="rounded-2xl border border-[#ddcfc2] bg-white px-4 py-2.5 text-sm font-medium text-stone-700">Refresh</button>
            </div>
        ...
        {activeTab === 'bookings' && (
          <div className="overflow-hidden rounded-[28px] border border-[#e7ddd3] bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-[#efe5db] px-6 py-4">
              ...
              <span className="text-sm text-stone-500">Live session table</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px]">
                <thead className="bg-[#faf6f1] text-left text-[11px] uppercase tracking-[0.18em] text-stone-500">
        ...
        {activeTab === 'history' && (
          <div className="overflow-hidden rounded-[28px] border border-[#e7ddd3] bg-white shadow-sm">
            <div className="border-b border-[#efe5db] px-6 py-4">
              <h2 className="text-xl font-semibold">Completed sessions</h2>
              <p className="mt-1 text-sm text-stone-500">{completedSessions.length} records</p>
            </div>
            <div className="overflow-x-auto">
        ...
        {activeTab === 'users' && (
          <div className="overflow-hidden rounded-[28px] border border-[#e7ddd3] bg-white shadow-sm">
            <div className="border-b border-[#efe5db] px-6 py-4">
              ...
              <p className="mt-1 text-sm text-stone-500">Update roles or remove accounts</p>
            </div>
            <div className="overflow-x-auto">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-[28px] border border-[#e7ddd3] bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 flex items-center justify-center">
                <div className="grid h-52 w-52 place-items-center rounded-full border-[18px] border-[#efe7de] text-center" style={{ boxShadow: `inset 0 0 0 18px rgba(124,79,100,${stats.occupancyRate / 170})` }}>
                  <div>
            ...
            <div className="rounded-[28px] border border-[#e7ddd3] bg-white p-6 shadow-sm">
              ...
              <div className="mt-5 space-y-3 text-sm">
                <div className="flex justify-between rounded-2xl bg-[#f4ede5] px-4 py-4"><span className="text-stone-600">Completed sessions</span><span className="font-semibold">${completedSessions.reduce((sum, s) => sum + (s.total_amount || 0), 0).toFixed(2)}</span></div>
                <div className="flex justify-between rounded-2xl bg-[#f4ede5] px-4 py-4"><span className="text-stone-600">Active session estimate</span><span className="font-semibold">${activeSessions.reduce((sum, s) => sum + s.duration_hours * 5, 0).toFixed(2)}</span></div>
                <div className="flex justify-between rounded-2xl bg-[#efe2e8] px-4 py-4"><span className="text-stone-700">Total</span><span className="font-semibold text-[#7c4f64]">${stats.todayRevenue.toFixed(2)}</span></div>
              </div>
            </div>
          </div>
        )}
