    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-900">
        <div className="text-center">
          <div className="mx-auto h-12 w-12 animate-spin rounded-full border-4 border-slate-700/60/20 border-t-[#465055]" />
          <p className="mt-4 text-sm uppercase tracking-[0.22em] text-slate-500">Loading staff panel</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-700/60 bg-slate-900/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div>
            <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">Operations</p>
        ...
        <div className="mx-auto flex max-w-6xl gap-2 px-4 pb-4 sm:px-6 lg:px-8">
          {[
            ...
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium ${activeTab === tab.id ? 'bg-slate-800 text-white' : 'border border-slate-700/60 bg-slate-900 text-slate-400'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        {activeTab === 'parking' && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              <div className="rounded-[24px] border border-slate-700/60 bg-slate-900 p-4 shadow-sm"><p className="text-xs uppercase tracking-[0.18em] text-slate-500">Total</p><p className="mt-2 text-3xl font-semibold">{spots.length}</p></div>
              <div className="rounded-[24px] border border-slate-700/60 bg-slate-900 p-4 shadow-sm"><p className="text-xs uppercase tracking-[0.18em] text-slate-500">Free</p><p className="mt-2 text-3xl font-semibold">{spots.filter((s) => !s.is_occupied && !s.is_reserved).length}</p></div>
              <div className="rounded-[24px] border border-slate-700/60 bg-slate-900 p-4 shadow-sm"><p className="text-xs uppercase tracking-[0.18em] text-slate-500">Occupied</p><p className="mt-2 text-3xl font-semibold">{spots.filter((s) => s.is_occupied).length}</p></div>
              <div className="rounded-[24px] border border-slate-700/60 bg-slate-900 p-4 shadow-sm"><p className="text-xs uppercase tracking-[0.18em] text-slate-500">Sessions</p><p className="mt-2 text-3xl font-semibold">{activeSessions.length}</p></div>
            </div>
        ...
        {activeTab === 'bookings' && (
          <div className="overflow-hidden rounded-[28px] border border-slate-700/60 bg-slate-900 shadow-sm">
            <div className="flex items-center justify-between border-b border-slate-700/60 px-6 py-4">
              ...
              <span className="text-sm text-slate-500">Monitor and extend sessions</span>
            </div>
