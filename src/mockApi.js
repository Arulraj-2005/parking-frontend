...
export const mockApi = {
  ...
  getSpots() {
    return JSON.parse(localStorage.getItem('sp_spots') || '[]');
  },
  saveSpots(spots) {
    localStorage.setItem('sp_spots', JSON.stringify(spots));
  },
  getSessions() {
    return JSON.parse(localStorage.getItem('sp_sessions') || '[]');
  },
  saveSessions(sessions) {
    localStorage.setItem('sp_sessions', JSON.stringify(sessions));
  },
  getVehicles() {
  ...
  async getZones() {
    ...
    const zones = spots.reduce((acc, spot) => {
      let zone = acc.find((x) => x.id === spot.zone_id);
      if (!zone) {
        zone = {
          id: spot.zone_id,
          name: spot.zone_name,
          description: `Zone ${spot.zone_name}`,
          total_spots: 0,
          available_spots: 0,
          occupied_spots: 0,
          reserved_spots: 0,
      ...
      zone.total_spots += 1;
      if (spot.is_occupied) zone.occupied_spots += 1;
      else if (spot.is_reserved) zone.reserved_spots += 1;
      else zone.available_spots += 1;
      return acc;
    }, []);
  ...
  async getDashboard() {
    ...
    const completedRevenue = sessions
      .filter((s) => s.duration_minutes > 0 && s.total_amount)
      .reduce((sum, s) => sum + (s.total_amount || 0), 0);

    const activeRevenue = sessions
      .filter((s) => s.duration_minutes === 0)
      .reduce((sum, s) => {
        const mins = (Date.now() - new Date(s.entry_time).getTime()) / 60000;
